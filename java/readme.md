...
The Execution Engine is a part of the Java Virtual Machine (JVM) responsible for executing the bytecode generated by the Java compiler.

It mainly consists of three components:

1. Interpreter : The Interpreter reads bytecode instruction by instruction. It translates each instruction into machine-level operations and executes it immediately.
~~~
bytecode → interpreter → machine instruction → CPU executes
~~~
2. JIT (Just-In-Time) Compiler : The JIT Compiler improves performance by identifying hotspots in the code. Hotspots are parts of code executed frequently (e.g., loops, frequently called methods). Instead of interpreting them every time, JIT converts those bytecode sections into native machine code. The compiled code is cached, so future executions run faster.
~~~
Frequently executed bytecode → JIT compiler → native machine code → Cach it
~~~
3. Garbage Collector (GC) : The Garbage Collector automatically detects and removes unused objects from the heap memory. It frees memory that is no longer referenced by any part of the program. This prevents memory leaks and manual memory management.
~~~
Object created → becomes unreachable → GC removes it from heap
~~~

### JNI (Java Native Interface)

Java Native Interface (JNI) is a framework that allows Java code running inside the JVM to call and interact with native code written in languages like C or C++.

It is needed to use :
1. Operating system features
2. Hardware resources
3. Existing C/C++ libraries
...
JNI
     ↓
C / C++ Native Code (Native method library)
     ↓
Operating System
~~~
...
Native Method Libraries are compiled libraries written in languages like C or C++ that contain the implementation of native methods.
~~~
Java Program
      ↓
Native Method (declared in Java)
      ↓
JNI
      ↓
